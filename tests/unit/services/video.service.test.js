...
describe("VideoService", () => {
    ...
    describe("createJob()", () => {
        it("should create a video job with correct parameters", async () => {
            ...
            database_1.prisma.video.create.mockResolvedValue(mockVideo);
            const params = {
                userId: BigInt(123),
                niche: "fnb",
                platform: "tiktok",
            ...
            const result = await video_service_1.VideoService.createJob(params);
            expect(pricing_1.getVideoCreditCost).toHaveBeenCalledWith(30);
            expect(database_1.prisma.video.create).toHaveBeenCalledWith({
            ...
            expect(logger_1.logger.info).toHaveBeenCalledWith(expect.stringContaining("Created video job:"));
        ...
        it("should generate default title when not provided", async () => {
            ...
            await video_service_1.VideoService.createJob(params);
            expect(database_1.prisma.video.create).toHaveBeenCalledWith({
                data: expect.objectContaining({
    ...
    describe("updateProgress()", () => {
        it("should update progress without status", async () => {
            database_1.prisma.video.update.mockResolvedValue({
                ...mockVideo,
                progress: 50,
            });
            const result = await video_service_1.VideoService.updateProgress("VID-123", 50);
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: {
                    progress: 50,
                    status: undefined,
        ...
        it("should update progress with status", async () => {
            ...
            await video_service_1.VideoService.updateProgress("VID-123", 75, "rendering");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: {
                    progress: 75,
        ...
        it("should set completedAt when status is completed", async () => {
            ...
            await video_service_1.VideoService.updateProgress("VID-123", 100, "completed");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: {
                    progress: 100,
    ...
    describe("setOutput()", () => {
        it("should set video output URLs and mark as completed", async () => {
            ...
            database_1.prisma.video.update.mockResolvedValue(updatedVideo);
            const urls = {
                videoUrl: "https://cdn.example.com/video.mp4",
                thumbnailUrl: "https://cdn.example.com/thumb.jpg",
                downloadUrl: "https://cdn.example.com/download.mp4",
            };
            const result = await video_service_1.VideoService.setOutput("VID-123", urls);
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: expect.objectContaining({
                    videoUrl: "https://cdn.example.com/video.mp4",
        ...
        it("should handle partial URLs", async () => {
            database_1.prisma.video.update.mockResolvedValue(mockVideo);
            await video_service_1.VideoService.setOutput("VID-123", {
                videoUrl: "https://cdn.example.com/video.mp4",
            });
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
    ...
    describe("updateStatus()", () => {
        it("should update status without error message", async () => {
            ...
            await video_service_1.VideoService.updateStatus("VID-123", "rendering");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: {
                    status: "rendering",
        ...
        it("should update status with error message", async () => {
            ...
            await video_service_1.VideoService.updateStatus("VID-123", "failed", "Provider error");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: {
                    status: "failed",
        ...
        it("should set completedAt and progress to 100 when status is completed", async () => {
            ...
            await video_service_1.VideoService.updateStatus("VID-123", "completed");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: expect.objectContaining({
                    status: "completed",
    ...
    describe("getByJobId()", () => {
        it("should return video when found", async () => {
            database_1.prisma.video.findUnique.mockResolvedValue(mockVideo);
            const result = await video_service_1.VideoService.getByJobId("VID-123");
            expect(database_1.prisma.video.findUnique).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
            });
            expect(result).toEqual(mockVideo);
        });
        it("should return null when video not found", async () => {
            database_1.prisma.video.findUnique.mockResolvedValue(null);
            const result = await video_service_1.VideoService.getByJobId("VID-NONEXISTENT");
    ...
    describe("deleteVideo()", () => {
        it("should soft delete video by setting status to deleted", async () => {
            ...
            await video_service_1.VideoService.deleteVideo("VID-123");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: { status: "deleted" },
            });
            expect(logger_1.logger.info).toHaveBeenCalledWith("Soft-deleted video: VID-123");
        });
    });
    // ── getUserVideos ──
    describe("getUserVideos()", () => {
        it("should return user videos with default limit", async () => {
            ...
            database_1.prisma.video.findMany.mockResolvedValue(videos);
            const result = await video_service_1.VideoService.getUserVideos(BigInt(123));
            expect(database_1.prisma.video.findMany).toHaveBeenCalledWith({
                where: {
                    userId: BigInt(123),
                    status: { not: "deleted" },
                },
        ...
        it("should respect custom limit", async () => {
            database_1.prisma.video.findMany.mockResolvedValue([mockVideo]);
            await video_service_1.VideoService.getUserVideos(BigInt(123), 5);
            expect(database_1.prisma.video.findMany).toHaveBeenCalledWith({
                where: {
                    userId: BigInt(123),
                    status: { not: "deleted" },
        ...
        it("should exclude deleted videos", async () => {
            database_1.prisma.video.findMany.mockResolvedValue([]);
            await video_service_1.VideoService.getUserVideos(BigInt(123));
            expect(database_1.prisma.video.findMany).toHaveBeenCalledWith({
    ...
    describe("generatePrompt()", () => {
        it("should generate prompt for known niche and platform", () => {
            const result = video_service_1.VideoService.generatePrompt({
                niche: "fnb",
                platform: "tiktok",
                duration: 30,
    ...
    describe("getCreditCost()", () => {
        it("should return credit cost for duration", () => {
            pricing_1.getVideoCreditCost.mockReturnValue(1.0);
            const result = video_service_1.VideoService.getCreditCost(30);
            expect(pricing_1.getVideoCreditCost).toHaveBeenCalledWith(30);
            expect(result).toBe(1.0);
        });
        it("should handle different durations", () => {
            pricing_1.getVideoCreditCost.mockReturnValue(0.5);
            const result = video_service_1.VideoService.getCreditCost(15);
            expect(pricing_1.getVideoCreditCost).toHaveBeenCalledWith(15);
    ...
    describe("getNiches()", () => {
        it("should return all available niches", () => {
            const result = video_service_1.VideoService.getNiches();
    ...
    describe("getPlatforms()", () => {
        it("should return all available platforms", () => {
            const result = video_service_1.VideoService.getPlatforms();
            expect(result).toEqual(expect.arrayContaining([
    ...
    describe("generateStoryboard()", () => {
        it("should generate storyboard for known niche", async () => {
            const result = await video_service_1.VideoService.generateStoryboard({
                niche: "fnb",
                duration: 15,
                productDescription: "Delicious burger",
            });
    ...
    describe("generateScenePrompt()", () => {
        it("should generate scene prompt with niche style", () => {
            const result = video_service_1.VideoService.generateScenePrompt({
                niche: "fnb",
                sceneType: "hook",
    ...
    describe("generateCaption()", () => {
        it("should generate Indonesian caption for fnb niche", async () => {
            const result = await video_service_1.VideoService.generateCaption({
                niche: "fnb",
                sceneCount: 4,
                language: "id",
            });
        ...
        it("should call Gemini API for non-id/en languages", async () => {
            ...
            languages_1.getAILabel.mockReturnValue("Thai");
            const result = await video_service_1.VideoService.generateCaption({
                niche: "fnb",
                sceneCount: 4,
                language: "th",
        ...
        it("should fallback to English when Gemini API fails", async () => {
            ...
            expect(logger_1.logger.warn).toHaveBeenCalledWith(expect.stringContaining("Gemini caption generation failed"));
            expect(typeof result).toBe("string");
            expect(result.length).toBeGreaterThan(0);
        });
    ...
    describe("getStoryboardTemplate()", () => {
        it("should return storyboard template for known niche", () => {
            const result = video_service_1.VideoService.getStoryboardTemplate("fnb");
        ...
        it("should return empty array for unknown niche", () => {
            const result = video_service_1.VideoService.getStoryboardTemplate("unknown");
            expect(Array.isArray(result)).toBe(true);
        ...
        it("should return templates for all known niches", () => {
            ...
            niches.forEach((niche) => {
                const result = video_service_1.VideoService.getStoryboardTemplate(niche);
                expect(Array.isArray(result)).toBe(true);
                expect(result.length).toBeGreaterThan(0);
            });
        });
    ...
    describe("updateStoryboardTemplate()", () => {
        it("should update storyboard template for known niche", () => {
            ...
            const result = video_service_1.VideoService.updateStoryboardTemplate("fnb", newScenes);
            expect(result).toBe(true);
            // Verify the template was updated
            const template = video_service_1.VideoService.getStoryboardTemplate("fnb");
        ...
        it("should return true even for unknown niche", () => {
            ...
            const result = video_service_1.VideoService.updateStoryboardTemplate("unknown", newScenes);
            expect(result).toBe(true);
        });
    });
    // ── processJob ──
    describe("processJob()", () => {
        it("should process job successfully", async () => {
            database_1.prisma.video.findUnique.mockResolvedValue(mockVideo);
            database_1.prisma.video.update.mockResolvedValue(mockVideo);
            video_generation_service_1.processVideoJob.mockResolvedValue({
                success: true,
                videoUrl: "https://cdn.example.com/video.mp4",
                thumbnailUrl: "https://cdn.example.com/thumb.jpg",
                provider: "geminigen",
            });
            await video_service_1.VideoService.processJob("VID-123");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
            ...
            expect(logger_1.logger.info).toHaveBeenCalledWith(expect.stringContaining("Video job completed"));
        });
        it("should handle job not found", async () => {
            database_1.prisma.video.findUnique.mockResolvedValue(null);
            await video_service_1.VideoService.processJob("VID-NONEXISTENT");
            expect(logger_1.logger.error).toHaveBeenCalledWith("Video job not found: VID-NONEXISTENT");
            expect(database_1.prisma.video.update).not.toHaveBeenCalled();
        });
        it("should handle provider failure", async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith(expect.stringContaining("Video job failed"));
        });
        it("should handle unexpected errors", async () => {
            database_1.prisma.video.findUnique.mockResolvedValue(mockVideo);
            database_1.prisma.video.update.mockResolvedValue(mockVideo);
            video_generation_service_1.processVideoJob.mockRejectedValue(new Error("Unexpected error"));
            await video_service_1.VideoService.processJob("VID-123");
            expect(database_1.prisma.video.update).toHaveBeenCalledWith({
                where: { jobId: "VID-123" },
                data: { status: "failed", errorMessage: "Unexpected error" },
            });
            expect(logger_1.logger.error).toHaveBeenCalledWith("Video job error: VID-123", expect.any(Error));
        });
    });
});
//# sourceMappingURL=video.service.test.js.map