...
const audio_vo_service_1 = require("@/services/audio-vo.service");
describe('AudioVOService', () => {
    describe('generateSRT()', () => {
        ...
        it('should produce valid SRT format with sequential indices', () => {
            audio_vo_service_1.AudioVOService.generateSRT(sampleBlocks, '/tmp/test.srt');
        ...
        it('should format timestamps in HH:MM:SS,mmm format', () => {
            ...
            audio_vo_service_1.AudioVOService.generateSRT(blocks, '/tmp/test2.srt');
            const [, content] = mockWriteFileSync.mock.calls[0];
        ...
        it('should handle zero-padded milliseconds correctly', () => {
            ...
            audio_vo_service_1.AudioVOService.generateSRT(blocks, '/tmp/test3.srt');
            const [, content] = mockWriteFileSync.mock.calls[0];
            // 0.05s = 50ms -> "050"
        ...
        it('should write to the specified output path', () => {
            ...
            const result = audio_vo_service_1.AudioVOService.generateSRT(sampleBlocks, outputPath);
            expect(result).toBe(outputPath);
            expect(mockWriteFileSync).toHaveBeenCalledWith(outputPath, expect.any(String), 'utf-8');
        });
        ...
        it('should handle empty blocks array gracefully', () => {
            audio_vo_service_1.AudioVOService.generateSRT([], '/tmp/empty.srt');
            const [, content] = mockWriteFileSync.mock.calls[0];
...
//# sourceMappingURL=audio-vo.test.js.map