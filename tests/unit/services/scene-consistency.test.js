...
const scene_consistency_service_1 = require("@/services/scene-consistency.service");
describe('SceneConsistencyEngine', () => {
    // ── createMemory ──
    describe('createMemory()', () => {
        it('should extract the main subject from the first clause of the prompt', () => {
            const memory = scene_consistency_service_1.SceneConsistencyEngine.createMemory('a beautiful leather handbag, on a marble countertop, golden hour light', 'fashion_lifestyle', 'realistic');
            expect(memory.mainSubject).toBe('a beautiful leather handbag');
        });
        it('should strip duration tokens from subject extraction', () => {
            const memory = scene_consistency_service_1.SceneConsistencyEngine.createMemory('10s smartphone on a desk, tiktok format, professional style', 'tech', 'realistic');
        ...
        it('should derive color palette from niche config when available', () => {
            const memory = scene_consistency_service_1.SceneConsistencyEngine.createMemory('red dress on a model', 'fashion_lifestyle', 'realistic');
            expect(memory.colorPalette).toBe('neutral tones');
        });
        it('should use fallback color palette for unknown niche', () => {
            const memory = scene_consistency_service_1.SceneConsistencyEngine.createMemory('a product on a table', 'tech', 'realistic');
        ...
        it('should set mood from the niche mood map', () => {
            const memory = scene_consistency_service_1.SceneConsistencyEngine.createMemory('nasi goreng close-up', 'fnb', 'realistic');
        ...
        it('should store the full first scene prompt in previousSceneDescription', () => {
            ...
            const memory = scene_consistency_service_1.SceneConsistencyEngine.createMemory(prompt, 'fashion_lifestyle', 'realistic');
    ...
    describe('enrichScenePrompt()', () => {
        ...
        it('should return the prompt unchanged for scene index 0', () => {
            ...
            const result = scene_consistency_service_1.SceneConsistencyEngine.enrichScenePrompt(original, memory, 0);
            expect(result).toBe(original);
        });
        it('should add continuity anchors for scene index > 0', () => {
            const scenePrompt = 'close-up of handbag stitching detail';
            const result = scene_consistency_service_1.SceneConsistencyEngine.enrichScenePrompt(scenePrompt, memory, 1);
        ...
        it('should update memory.previousSceneDescription after enrichment', () => {
            ...
            scene_consistency_service_1.SceneConsistencyEngine.enrichScenePrompt(scenePrompt, memory, 1);
        ...
        it('should chain scene descriptions across multiple scenes', () => {
            ...
            scene_consistency_service_1.SceneConsistencyEngine.enrichScenePrompt(scene2, memory, 1);
            expect(memory.previousSceneDescription).toBe(scene2);
            const scene3 = 'lifestyle shot with model';
            const result = scene_consistency_service_1.SceneConsistencyEngine.enrichScenePrompt(scene3, memory, 2);
            // Scene 3 should reference scene 2's prompt as previous
            expect(result).toContain('detail shot of zipper');
        });
    });
});
//# sourceMappingURL=scene-consistency.test.js.map