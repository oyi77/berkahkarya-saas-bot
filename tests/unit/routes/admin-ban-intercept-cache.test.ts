...
const mockUserUpdate = jest.fn().mockResolvedValue({ isBanned: true });
...
describe('Admin ban endpoint cache coherence', () => {
  let app: any;
  ...
  beforeEach(() => {
    ...
    mockUserUpdate.mockResolvedValue({ isBanned: true });
  });

  it('invalidates intercept cache after ban toggle', async () => {
    const telegramId = '123456789';
    const res = await request(app.server)
      .post(`/api/users/${telegramId}/ban`)
      .set('Authorization', adminBasicAuth())
      .send({ banned: true, reason: 'policy violation' });
    ...
    expect(mockInvalidateCache).toHaveBeenCalledWith(BigInt(telegramId));
  });
});
