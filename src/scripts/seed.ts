...
export async function runSeeder() {
  try {
    ...
    const { PaymentSettingsService } = await import('../services/payment-settings.service.js');
    ...
    for (const niche of NICHE_LIST) {
      await prisma.pricingConfig.upsert({
        ...
        create: { category: 'niche', key: niche.id, value: niche, updatedAt: new Date() },
    ...
    await prisma.pricingConfig.upsert({
      ...
      create: { category: 'free_trial', key: 'config', value: FREE_TRIAL_CONFIG },
      update: {},
    });
    ...
    const defaultLandingConfig = {
      ...
      subheadline: 'Platform AI yang ubah foto produk jadi video iklan profesional dengan struktur HPAS. Langsung dari Telegram. Mulai gratis, bayar pakai VA/e-wallet.',
      videoDuration: '60',
    ...
    await redis.set('admin:landing_config', JSON.stringify(defaultLandingConfig));
    logger.info('[SEEDER] Successfully seeded default admin:landing_config with multi-language testimonials.');
  } catch (error: any) {
...
if (require.main === module) {
  runSeeder().then(() => process.exit(0)).catch(() => process.exit(1));
}
