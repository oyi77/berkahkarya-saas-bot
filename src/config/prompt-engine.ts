...
const CAMERA_PRESETS = {
  product_hero: 'shot on Canon EOS R5, 85mm lens, f/2.8, ISO 200, sharp focus on product, shallow depth of field',
  product_detail: 'shot on Canon EOS R5, 100mm macro lens, f/5.6, ISO 400, extreme detail, tack sharp',
  lifestyle: 'shot on Sony A7IV, 50mm lens, f/2.0, ISO 400, natural perspective, bokeh background',
  food_overhead: 'shot on Canon EOS R5, 35mm lens, f/4.0, ISO 320, top-down flat lay, even sharpness',
  food_closeup: 'shot on Sony A7IV, 85mm lens, f/1.8, ISO 250, selective focus on hero element, steam visible',
  architecture: 'shot on Sony A7RIV, 16-35mm f/2.8, ISO 100, wide angle, leading lines, deep depth of field',
  automotive: 'shot on Canon EOS R5, 70-200mm f/2.8, ISO 200, dramatic angle, reflections visible',
  portrait: 'shot on Sony A7IV, 85mm f/1.4, ISO 320, eye-level, catchlights in eyes, skin detail preserved',
} as const;

const LIGHTING_PRESETS = {
  studio_product: 'professional studio lighting, large softbox camera-left, fill light camera-right, rim light for edge separation, even illumination, no harsh shadows',
  natural_warm: 'golden hour natural light, soft warm tones, gentle shadows, color temperature 5500K, ambient fill',
  food_appetizing: 'warm directional light from side, highlighting steam and texture, color temperature 3500K warm, soft shadows to create depth, backlight for translucency',
  real_estate: 'bright natural daylight, wide-angle lighting, HDR-balanced exposure, all areas visible, window light dominant',
  automotive_dramatic: 'dramatic studio lighting, sharp edge lights, dark moody backdrop, specular highlights on paint, controlled reflections',
  commercial_clean: 'even diffused lighting, minimal shadows, bright key light, white background illumination, commercial polish',
  lifestyle_natural: 'soft natural window light, slight warm cast, gentle shadows, authentic feel, not over-lit',
} as const;

const STYLE_SIGNATURES = {
  hyperreal_commercial: 'photorealistic, 8K resolution, commercial-grade quality, magazine-worthy composition, professional color grading, Kodak Portra 400 emulation',
  ads_ready: 'advertising photography, product clearly visible, hero placement, clean composition, aspirational mood, brand-safe, no text overlays',
  content_post: 'social media optimized, scroll-stopping composition, vibrant but natural colors, Instagram aesthetic, engagement-optimized framing',
  premium_product: 'luxury product photography, meticulous detail, premium feel, sophisticated color palette, high-end retouching, gallery-worthy',
  food_commercial: 'food styling photography, appetizing color science, steam and freshness emphasized, restaurant-quality plating, warm inviting mood',
  architectural_luxury: 'architectural photography, spacious feel, luxury finish visible, bright airy atmosphere, aspirational lifestyle',
} as const;

const NEGATIVE_PROMPTS = {
  universal: 'blurry, low quality, distorted, deformed, ugly, duplicate, watermark, text overlay, logo, signature, cropped, out of frame, worst quality, low resolution, jpeg artifacts, noise, grain',
  product: 'cartoon, anime, illustration, painting, 3d render, artificial looking, floating objects, unrealistic shadows, hands holding incorrectly, wrong proportions',
  ...
  realestate: 'cluttered, messy, small cramped space, poor lighting, distorted walls, fisheye distortion, unrealistic furniture scale, empty lifeless rooms',
  ...
  person: 'extra fingers, extra limbs, deformed face, asymmetric eyes, uncanny valley, plastic skin, mannequin-like, zombie appearance',
} as const;
...
const CATEGORY_TEMPLATES: Record<string, CategoryTemplate> = {
  product: {
    ...
    environmentDefault: 'clean minimal studio environment, subtle gradient background, reflective surface for premium feel, no distracting elements',
    lightingPreset: 'studio_product',
    cameraPreset: 'product_hero',
    styleSignature: 'premium_product',
    negativePreset: 'product',
    qualityBoost: 'advertising campaign quality, e-commerce hero shot, ready for product listing and social media ads',
  },
  fnb: {
    subjectEnricher: (desc) =>
    ...
    environmentDefault: 'warm restaurant or kitchen setting, wooden surface or marble countertop, subtle props (herbs, spices, utensils) for context, depth visible in background',
    lightingPreset: 'food_appetizing',
    cameraPreset: 'food_closeup',
    styleSignature: 'food_commercial',
    negativePreset: 'food',
    qualityBoost: 'food magazine cover quality, appetizing enough to taste, ready for restaurant menu and food delivery app',
  },
  realestate: {
    subjectEnricher: (desc) =>
      `${desc}, spacious and inviting composition, luxury finishes visible, well-maintained and staged, aspirational living space`,
    environmentDefault: 'bright modern interior or stunning exterior, lush landscaping or contemporary furnishings, high ceilings and open floor plan feel',
    lightingPreset: 'real_estate',
    cameraPreset: 'architecture',
    styleSignature: 'architectural_luxury',
    negativePreset: 'realestate',
    qualityBoost: 'real estate listing hero photo quality, MLS-ready, property portal optimized, makes viewer want to schedule a viewing',
  },
  car: {
    subjectEnricher: (desc) =>
    ...
    environmentDefault: 'dramatic environment complementing the vehicle, either clean studio with dark backdrop or scenic outdoor location, reflective floor surface',
    lightingPreset: 'automotive_dramatic',
    cameraPreset: 'automotive',
    styleSignature: 'hyperreal_commercial',
    negativePreset: 'car',
    qualityBoost: 'automotive magazine cover quality, dealership-ready hero shot, ready for car listing and social media showcase',
  },
};
...
export class PromptEngine {
  ...
  static enrichForVideo(
    ...
    const sceneLabel = sceneNumber && totalScenes
    ...
    const motionDirections: Record<string, string> = {
      ...
      fashion: 'smooth tracking shot, model movement with confidence, elegant slow-motion details',
      ...
      travel: 'epic establishing shot with slow reveal, cinematic drone-like movement, golden hour magic',
      education: 'steady focus shot, clean minimal movement, professional presentation',
      finance: 'subtle push-in for emphasis, professional steady cam, trust-building visual pace',
      entertainment: 'dynamic quick cuts, playful camera angles, high energy movement',
    };
  ...
  static getNegativePrompt(category: string): string {
    const catNeg = NEGATIVE_PROMPTS[category as keyof typeof NEGATIVE_PROMPTS] || '';
